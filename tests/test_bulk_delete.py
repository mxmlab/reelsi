...
"""Удаление выбранных клипов и выбор диапазона через Shift.
...
def _attrs(open_tag):
    ...
    разметки (на этом старый сторож пропускал любую подмену).
    """
...
def test_disk_delete_calls_every_clip_and_survives_one_error(tmp_path):
    ...
    Бэкенд удаляет по одному XML (цикл во фронте), поэтому клипов в цикле
...
def test_delete_selected_button_is_dead_with_empty_selection(tmp_path):
    ...
    берётся из свойства `disabled`, которое выставляет боевая функция.
    """
