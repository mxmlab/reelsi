...
1. Режим строк (`sub_words_per_row=2`): раньше жёлтое слово ложилось статично на
   ...
   короткой строки (видимое время до `r_t1`, поле 3 слова данных),
...
# появление играет 0.21 с); 11 — «МЕТКОНСЕКТ», второй в длинной строке
...
def _stand_code(jsx):
    ...
    # цикл строк — hlRowDur(wd). Стенду они нужны так же, как
...
def _wd_dur(wd, dur):
    """Длительность появления жёлтого слова строки: своя (поле 3) или общая —
...
def test_rows_word_anim_yellow_appears_when_spoken(xml_subs, tmp_path):
    ...
    а длительность появления — своя у короткой строки и общая у длинной."""
    ...
    for lay, (row, wd) in zip(run["layers"], pairs):
        ...
        # Длительность появления: та же функция, что у MA, — видимое время считается
...
def test_rows_row_anim_yellow_appears_with_row(xml_subs, tmp_path):
    ...
    for lay, (row, wd) in zip(run["layers"], pairs):
        ...
        assert lay["inPoint"] == pytest.approx(r_t0)
        # Длительность появления — как в цикле: своя у короткой строки, общая у длинной.
...
def test_blur_keys_in_all_three_loops(xml_subs, tmp_path, mode):
    ...
    for lay in yellow:
        ...
        t0 = lay["inPoint"]
        # Подъём, проявление и блюр стоят на ОДНИХ ключах (контракт ZH).
