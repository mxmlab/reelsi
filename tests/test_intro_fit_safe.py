...
"""Откреплённое интро: ручка видна по делу, потолок увеличения, безопасная зона.
...
2. Блок откреплённого интро стоял ВЫШЕ безопасной линии: стиль по умолчанию, `intro_cam=False`,
 ...
 ) и едет в тот же iDy/y, что уходят в .jsx (INTRO_IDY) и в план (y) — превью
   покажет то же.
...
def _old_top(plan, anchor):
    """Верх блока, каким он был ДО этого: опускание считала только _intro_i_dy
