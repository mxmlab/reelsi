...
"""Широкие интро-прекомпы (правка 2026-08-08) и переезд автофита в Python (BP).
...
(задание BP) считает scene_plan: в шаблоне остался только масштаб слоя от
...
def test_autofit_moved_to_python():
    """Автофит убрали из шаблона (задание BP): шаблон применяет только масштаб слоя
