...
r"""Сторож дрейфа документации (задание ND, журнал заданий).
...
Отдельно от этого файла живёт `tests/test_docs_freshness.py` (задание BN):
...
# `docs/*.md` и `README*.md` их не ловят. Собираются из частей НАРОЧНО: сам этот
# файл уезжает в публичный срез, а те документы в срез не входят — литеральное
# имя непубликуемого пути в публикуемом файле и есть отсылка в пустоту (ловит
# `tests/test_review_fixes.py::test_public_files_have_no_dangling_links`).
_ROOT_AGENT_DOCS = ("CLAUDE" + ".md", "AGENTS" + ".md")

# Каталог архива закрытых аудитов и реализованных планов: история, а не карта
# кода. Имя тоже собрано из частей — по той же причине, что и выше.
...
def _doc_paths():
    ...
    попадает, и это здесь же проверяется утверждением (задание ND, п. 1)."""
...
def test_all_required_docs_exist():
    ...
    not_checked = [rel for rel in REQUIRED_DOCS if rel not in checked]
    assert not not_checked, f"документы есть, но обход их не берёт: {not_checked}"
    assert len(checked) >= len(REQUIRED_DOCS), (
        f"обход проверил меньше документов, чем в списке: {len(checked)} < {len(REQUIRED_DOCS)}")
