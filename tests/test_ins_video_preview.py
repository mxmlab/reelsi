...
"""Видеовставки в превью шага 2: без моргания чёрным и с рабочим «Масштаб, %» (задание ME).
...
1. **Моргание.** Перестройка оверлея (`ipvOverlayPlan`/`ipvOverlay`) на каждый новый план
   ...
   `load`) — иначе каждый держит соединение (задание MB).
...
(`_fill_slack`/`insVideoPan`) — свободная при любом масштабе (задание ME2): позиция =
...
def test_vanished_insert_releases_its_video():
    ...
    Иначе каждый просмотренный файл висит соединением (задание MB: браузер держит 6
...
def test_scale_above_100_zooms_the_frame():
    ...
    Здесь же стережётся панорама: клампа запасом вылета больше нет (задание ME2) — x/y
    ...
    # режут — было `translate(slackx px,0px)`, стало ровно заданное (задание ME2)
