...
# считаться в Python до сборки .jsx (задание B). Значения не менялись.
...
# полного исчезновения (outEnd) оставлен прежним, а начало фейда сдвинуто позже (задание IK).
...
INTRO_FIT_W = 0.92
# Затемнение под интро (задание IL): числа сняты с amdi1.aep — пользователь в КАЖДОМ из
...
def _intro_i_dy(h, n_lines, gs, step_k=1.0):
    """Опускание блока интро под INTRO_SAFE_TOP, px (задание Q2). Масштаб группы
    (96.8·gs/100) выбирает вызывающий: у ПРИВЯЗАННОГО интро это НЕужатый gs (граница
    задания BP — там автофит режет только Scale, и опускание от него не зависит), у
    ОТКРЕПЛЁННОГО (задание MI) — фактический ds, который автофит мог и увеличить:
...
# (intro_line_sizes), верх строки — капитель (_cap, задание ZY), низ — выносные (_desc).
...
def intro_line_sizes(lines, fsize, back_scale=1.0, lk=None):
    """Кегль каждой строки интро, px (задание MH): fsize у обычной строки,
    fsize·back_scale у строки заднего плана, fsize·lk у большой строки (задание ZY).
...
def _desc(ps, size):
    """Глубина выносных под базовой линией, px (задание MH): низ «y» из контуров глифа.
...
def intro_block_span(ys, sizes, h, ds=100.0, g=100.0, y=0.0, dy=0.0,
    ...
    (задание MH). Верх блока — по капители самой верхней строки, низ — по выносным
    ...
    в процентах на нужный момент; intro_cam=False (интро откреплено, задание ZM) — зум
...
def sub_span(posy, size, font=None, row=0, step=0.0):
    """(top, bottom) — вертикальная полоса субтитров в кадре, px от верха кадра (задание MH).
...
def spans_hit(a, b):
    """Пересекаются ли две вертикальные полосы (задание MH). Касание краями — не
...
def intro_hits_subs(ys, sizes, posy, sub_size, h=1920.0, ds=100.0, g=100.0,
    ...
    """Стоит ли блок интро на полосе субтитров (задание MH), True/False.
...
def intro_sub_window(in_at, te, next_sub, sub_fade, f_in=INTRO_F_DUR):
    """(te, fade, fade_start) — окно группы, гаснущей к появлению субтитра (задание MH).
...
def _fill_slack(iw, ih, comp_w, comp_h, k):
    ...
    видеовставки двигают свободно при любом масштабе (задание ME2) — уехав за край
