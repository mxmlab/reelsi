...
"""Безголовый рендер в AE (шаг 3, api/render.py).
...
И предполётная проверка ДО AE: она обязана пропустить клип с интро и
...
def test_progress_parses_frame_in_parens():
    ...
    assert aerender.parse_progress("(1/1600) frame", 1600) == pytest.approx(1 / 1600)
    assert aerender.parse_progress("(800/1600) frame", 1600) == pytest.approx(0.5)
    assert aerender.parse_progress("(10) frame", 1600) == pytest.approx(10 / 1600)
    # без total в строке — берём total из плана (второй аргумент)
    assert aerender.parse_progress("(640)", 1600) == pytest.approx(0.4)


def test_progress_marks_end():
    """Finished composition / Total Time Elapsed — маркеры конца: 100%."""
    assert aerender.parse_progress("Finished composition: MAIN", 1600) == 1.0
    assert aerender.parse_progress("Total Time Elapsed: 161.8 seconds", 1600) == 1.0


def test_progress_ignores_non_frame_lines():
    assert aerender.parse_progress("After Effects 26.2 - running...", 1600) is None
    assert aerender.parse_progress("", 1600) is None
...
def test_comp_frames_from_plan():
    ...
    assert aerender.comp_frames(jsx) == 1980            # 33с * 60
    jsx2 = jsx.replace("Math.max(DUR,1)", "Math.max(DUR,1)+1.35")
    assert aerender.comp_frames(jsx2) == round((33.0 + 1.35) * 60)
    assert aerender.comp_frames("no fps here") is None


def test_default_render_dir_is_next_to_repo():
    """Папка вывода по умолчанию — exp РЯДОМ с репозиторием (он публичный), не внутри."""
    d = aerender.default_render_dir()
...
def test_find_ae_returns_none_when_missing(tmp_path, monkeypatch):
    monkeypatch.setenv("ProgramFiles", str(tmp_path))
    assert aerender.find_ae() is None
...
def _preflight(jobs, tmp_path, monkeypatch):
    ...
    Набор нормализуем сами: в бою это делает api_render_run ДО ответа,
...
def test_preflight_passes_for_ui_intro_and_inserts(tmp_path, monkeypatch):
    """Клип с интро и вставками из интерфейса проходит предполёт: рендер
    ...
    # это ловушка: появится работа между вызовами, состояние молча сотрётся.
...
def test_preflight_blocks_missing_file(tmp_path, monkeypatch):
    ...
    предполёт и существует (-noui пишет пропажу в $.writeln и уходит в
...
def test_short_path_has_no_spaces(tmp_path):
    """Короткое 8.3-имя — без пробелов по построению; на томе без 8.3 — None."""
    d = str(tmp_path / "папка с пробелом")
    os.makedirs(d)
    p = os.path.join(d, "01 РИЛС 12.jsx")
    open(p, "w").write("x")
    sp = aerender.short_path(p)
...
def test_jsx_call_path_falls_back_to_spacefree_copy(tmp_path, monkeypatch):
    ...
    не молчим: иначе рендер тихо не запустится. Путь к .aep при этом не
    меняется — он задан явной константой в хвосте."""
    monkeypatch.setattr(aerender, "short_path", lambda p: None)
    d = str(tmp_path / "out")
    os.makedirs(d)
    jp = os.path.join(d, "01 РИЛС 12.jsx")
    open(jp, "w").write("var x=1;")
    call, why = aerender.jsx_call_path(jp)
...
def test_aep_call_path_short_when_available(tmp_path, monkeypatch):
    """Есть короткое 8.3-имя — зовём его, копию не создаём, причины нет."""
    d = str(tmp_path / "dir")
    os.makedirs(d)
    aep = os.path.join(d, "03_РИЛС 3 22.08.aep")
    open(aep, "w").write("x")
    if not aerender.short_path(aep):
        pytest.skip("том не поддерживает 8.3-имена (или не Windows)")
    call, why = aerender.aep_call_path(aep)
...
def test_aep_call_path_falls_back_to_ascii_copy(tmp_path, monkeypatch):
    ...
    непустая: aerender читает -project как ANSI, и на кодовой странице 1252
...
def test_aep_call_path_hashes_distinct_cyrillic_stems(tmp_path, monkeypatch):
    ...
    open(a1, "w").write("x")
    open(a2, "w").write("x")
    c1, _ = aerender.aep_call_path(a1)
    c2, _ = aerender.aep_call_path(a2)
    assert c1 != c2


def test_rendered_ok_requires_real_file(tmp_path):
    """«Готово» только по факту файла: отсутствующий и нулевой — False, непустой — True.
    aerender вернул 0 на «Path is not valid», коду возврата не верим."""
    miss = str(tmp_path / "нет.mov")
    empty = str(tmp_path / "empty.mov")
    full = str(tmp_path / "full.mov")
    open(empty, "w").close()
    open(full, "w").write("x")
    assert not aerender.rendered_ok(miss)
    assert not aerender.rendered_ok(empty)
    assert aerender.rendered_ok(full)


def test_render_status_exposes_items():
    """/api/render_status отдаёт items наравне с остальным состоянием."""
    import api
...
def test_parse_output_to_real_aerender_string():
    """Имя композиции из Output To:. Имя выходного файла — факт о композиции
    В НАЧАЛЕ её рендера."""
    line = r"PROGRESS:  Output To: C:\путь\C0233.mov"
    assert aerender.parse_output_to(line) == "C0233"
    assert aerender.parse_output_to("PROGRESS:  Output To: C:/render/C0233.mov") == "C0233"
    assert aerender.parse_output_to(r'PROGRESS:  Output To: "C:\output\C0233.mov"') == "C0233"
    assert aerender.parse_output_to("PROGRESS: Launching After Effects...") is None


def test_parse_ae_header_frames_real_aerender_block():
    """Число кадров композиции — от AE, из блока Start/End/Duration/Frame Rate."""
    ...
    fps = aerender.parse_frame_rate(line_fps)
    assert fps == 60.0

    m_dur = aerender.TC_DUR.search(line_dur)
    assert m_dur and m_dur.group(1) == "0:00:00:03"
    assert aerender.parse_timecode(m_dur.group(1), fps) == 3

    m_start = aerender.TC_START.search(line_start)
    m_end = aerender.TC_END.search(line_end)
    assert m_start and m_end
    sf = aerender.parse_timecode(m_start.group(1), fps)
    ef = aerender.parse_timecode(m_end.group(1), fps)
    assert sf == 0
    assert ef == 2
    assert ef - sf + 1 == 3
...
def test_finished_comp_name_real_aerender_string():
    """Finished composition из реального вывода: имя без кавычек, точки и даты."""
    line = 'PROGRESS:  8/26/2026 1:09:51 AM: Finished composition "C0233".'
    assert aerender.finished_comp_name(line) == "C0233"


def test_frame_number_starts_at_1():
    """Номер кадра в скобках начинается с 1 (0:00:00:00 (1)), 3 кадра -> 1/3, 2/3, 1.0."""
    assert aerender.parse_progress("PROGRESS:  0:00:00:00 (1): 0 Seconds", 3) == pytest.approx(1 / 3)
    assert aerender.parse_progress("PROGRESS:  0:00:00:01 (2): 0 Seconds", 3) == pytest.approx(2 / 3)
    assert aerender.parse_progress("PROGRESS:  0:00:00:02 (3): 0 Seconds", 3) == pytest.approx(3 / 3)


def test_run_proc_batch_identifies_comp_from_output_to_and_header():
    """_run_proc_batch определяет имя из Output To: и кадры из блока AE на реальном выводе aerender."""
    raw_lines = [
