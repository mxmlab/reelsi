...
Поля пресета (все опциональны, чего нет — берётся из «Базового»):
  ...
  intro_last_hold     сколько держать ПОСЛЕДНЮЮ группу интро после её последнего слова, с
                 (дефолт 1.0 = прежняя константа INTRO_HOLD; 0 — последняя гаснет ровно
                 как остальные группы, и надбавка intro_fx_hold_add к ней не применяется)
  ...
  intro_glitch_shadow галка: тень на словах с анимацией glitch (дефолт True)
...
BASE: dict[str, Any] = {
    ...
    "insert_c1_x": 0.0,                    # общий сдвиг точки покоя вставок Кам1 по X, px (0 = как сегодня);
                                           ...
                                           # поэтому и сдвиг масштабируется при наезде
    ...
    "insert_plate_file": None,             # файл картинки-подложки для вставок с галкой «на подложке»
                                           # решение — у вставки (поле plate), не у стиля
    "insert_plate_scale": 100.0,           # масштаб подложки, % (30-300 в UI)
    "layer_order": ["subs", "video", "roto", "photo", "intro"], # порядок слоёв сверху вниз
    ...
    "back_step_after": None,               # шаг ОТ заднего плана к обычной строке под ним, доля
                                           # от обычного шага; None = ключа нет —
    ...
    "intro_big_gap": 40.0,                 # зазор между большим словом слева и стопкой строк
                                           # справа, px: строку с галкой «большое
    ...
    "intro_scale_anchor": "comp",          # точка, от которой прекомп интро уменьшается и
                                           # увеличивается (задание «якорь масштаба»): "comp" —
                                           # центр композиции прекомпа (как раньше), "first" —
                                           # Y первой строки блока, "block" — середина между
                                           # первой и последней строкой. Якорь слоя ставит
    ...
    # Полка ПОСЛЕДНЕЙ группы интро: сколько держать её после последнего слова. Владелец:
    # «Почему последнее интро всё ещё длинное» — последняя группа висела на F_DUR+HOLD
    # (1.3 с) дольше остальных, и подрезать её под старт следующей не подо что. Дефолт 1.0 —
    # прежняя константа INTRO_HOLD, поэтому .jsx стиля без ключа собирается байт в байт
    # прежним (golden_geometry.jsx). 0 — последняя считается ровно как непоследние
    # (max(gmax, in_at+F_DUR)) и надбавки intro_fx_hold_add не получает (план, _far).
    "intro_last_hold": 1.0,
    ...
    # эффект или нет» было зашито в шаблон: строки с anim=="glitch" и строки заднего плана
    # получали Drop Shadow всегда, строки с глитчем и с fx=="glow" — всегда Glo2, и повторить
    # вид проекта, где галки этих эффектов в AE сняты руками, пересборкой было нельзя.
    # Галка выключена — эффект в .jsx НЕ добавляется вовсе (лишний выключенный эффект в AE
    # только мешает). Дефолты True у всех четырёх — прежнее поведение: .jsx без этих ключей
    # собирается байт в байт как раньше (golden_geometry.jsx).
    "intro_glitch_shadow": True,           # тень (Drop Shadow) на словах с анимацией glitch
    ...
    # прекомп. Дефолты True — .jsx без этих ключей собирается байт в байт как раньше
    # (golden_geometry.jsx).
    "intro_hl_glow": True,                 # свечение Glo2 на жёлтых словах хайлайта
    "intro_comp_glow": True,               # свечение Glo2 на слое прекомпа группы
    "intro_word_glow_thr": 149,            # Glo2 Threshold на словах, 0..255 (было жёстко 149)
    "intro_word_glow_rad": 77,             # Glo2 Radius на словах, 0..500 px (было жёстко 77)
    "intro_word_glow_int": 0.62,           # Glo2 Intensity на словах, 0..10 (было жёстко 0.62)
    # Тень ПРЕКОМПА интро: своя у камеры 1 и у камеры 2 (нул «интро на кам2»).
    ...
    # Затемнение под интро: в каждом из четырёх роликов amdi1.aep пользователь
    ...
    "cam1_head_min": 0,                    # следить от зума, % (0 = всегда)
    # Цвет камер через Lumetri: один эффект ADBE Lumetri на все клипы камер и
    ...
    "caption_size": 26.0,                  # кегль текста на экране, px (единственный размер, масштаб слоя 100%)
    "caption_case": "upper",               # регистр подписи: upper | as-is (дефолт upper)
    ...
    "caption_kx": 1.718,                   # множитель ширины плашки от видимого текста ()
    "caption_ky": 2.484,                   # множитель высоты плашки от видимого текста ()
}
