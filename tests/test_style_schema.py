...
def test_every_base_key_accounted_for():
    """styles.BASE имеет ровно 158 ключей; каждый в схеме или в EXTERNAL (без списков исключений)."""
    base_keys = set(styles.BASE.keys())
    assert len(base_keys) == 158, f"Ожидалось ровно 158 ключей в BASE, найдено {len(base_keys)}"
    ...
    # Все ключи полей уникальны между собой (146 полей, 147 ключей: cam1_zoom_cx/cy)
...
def test_api_style_schema_endpoint(client):
    """GET /api/style_schema отдаёт ok: True, layers (10), external (2), base, 39 групп, 146 полей."""
