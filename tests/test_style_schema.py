...
Чисел здесь нет намеренно (задание NA): «ровно 159 ключей», «39 групп», «147 полей» —
...
def test_base_and_schema_cover_each_other():
    ...
    его не поменять — так девять ключей и прожили без ручки до задания JB); ключ есть в
