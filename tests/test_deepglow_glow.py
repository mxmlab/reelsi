...
"""Deep Glow не ставится на строку со «свечением».
...
же порогом TRITONE_MAX_LUM, что у Tritone: на ярком цвете свечение выбеливает
...
def _tritone_expected(jsx):
    """Tritone, который на тёмном цвете идёт за жёлтой строкой."""
...
def test_2_без_свечения_deep_glow_как_сейчас(xml_subs, tmp_path):
    ...
    # И то же на сборке без свечения вовсе — ровно прежнее поведение.
...
def test_4_сторож_каждая_ручка_знает_новую_галку():
    ...
    Числа ключей/полей/групп здесь не стерегутся: перечень ручек сторож
...
def test_5_яркий_цвет_жёлтого_deep_glow_не_ставится(xml_subs, tmp_path):
    """5. Яркий ЗАДАННЫЙ цвет жёлтого (доработка MK3; [1,0.97,0.08] — яркость
...
def test_7_яркость_считается_той_же_функцией_и_порогом():
    """7. Проверка яркости у Deep Glow — та же, что у Tritone (доработка MK3):
